...
struct SeedsView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                // Title
                HeaderElement(title: .seedsTitle, withBackButton: true, withImage: true) {
                    dismiss()
                }
                .frame(height: 70)
    ...
    private var seedItems: [ConsumableModel] {
        return consumableService.getConsumables(ofType: .seeds)
    }
}

struct SeedItemRow: View {
